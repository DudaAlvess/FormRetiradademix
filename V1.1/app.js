...
const demoProducts = [
  {
    ...
    stockQuantity: 1240,
    unitOfMeasure: "UN",
    rawMaterialUsed: "Filme laminado",
  ...
  },
  {
    ...
    stockQuantity: 96,
    unitOfMeasure: "CX",
    rawMaterialUsed: "Papelao ondulado",
...
function bindEvents() {
  ...
  els.productForm.addEventListener("submit", (event) => {
    ...
    product.monthlyConsumption = Number(product.monthlyConsumption || 0);
...
function renderProducts() {
  ...
  const products = state.products.filter((product) => {
    const haystack = normalize(`${product.productId} ${product.name} ${product.rawMaterialUsed}`);
  ...
  els.productRows.innerHTML = products.map((product) => `
    <tr>
      ...
      <td>${escapeHtml(product.rawMaterialUsed || "-")}</td>
      <td>${number(product.monthlyConsumption)}</td>
    </tr>
  `).join("");
}
...
function renderDeclaration(request) {
  ...
  const rows = [
    ...
    ["Materia-prima utilizada", product.rawMaterialUsed || "-"],
    ["Consumo mensal estimado", number(product.monthlyConsumption)],
...
function rowsToProducts(headers, rows) {
  return rows.map((row) => {
    ...
    return {
      ...
      rawMaterialUsed: item["materia prima"] || item["raw material used"] || item["materia-prima utilizada"] || "",
      monthlyConsumption: Number(item["consumo mensal"] || item["monthly consumption"] || item["consumo mensal estimado"] || 0)
    };
...
function exportCsv() {
  const header = ["codigo", "produto", "estoque", "unidade", "materia prima", "consumo mensal"];
  const rows = state.products.map((product) => [
    product.productId,
    product.name,
    product.stockQuantity,
    product.unitOfMeasure,
    product.rawMaterialUsed,
