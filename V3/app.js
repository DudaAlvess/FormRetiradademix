...
const els = {
  ...
  ocOpenSelect: document.getElementById("ocOpenSelect"),
  ocFields: document.getElementById("ocFields"),
...
function bindEvents() {
  ...
  els.requestForm.addEventListener("submit", (event) => {
    ...
    const request = {
      ...
      fields: {
        ...
        ocOpen: data.ocOpen,
        ocNumber: data.ocNumber ? data.ocNumber.trim() : "",
        ocValue: Number(data.ocValue || 0),
        ocQuantity: Number(data.ocQuantity || 0)
      }
    };
  ...
  els.ocOpenSelect.addEventListener("change", toggleOcFields);
  toggleOcFields(); // estado inicial
...
function renderDeclaration(request) {
  ...
  const rows = [
    ...
    ...(fields.ocOpen === "Sim" ? [
      ...
      ["Quantidade da OC", number(fields.ocQuantity)]
    ] : [])
  ];
...
function toggleOcFields() {
  const isOpen = els.ocOpenSelect.value === "Sim";
  els.ocFields.classList.toggle("hidden", !isOpen);
}
